...
export const advanceDay = (gameState: GameState): GameState => {
  const dailyExpenses = 100 + (gameState.properties.filter(p => p.ownerId === gameState.character.id).length * 50);
...
const handleEndOfLife = (gameState: GameState): GameState => {
  const ownedProperties = gameState.properties.filter(p => p.ownerId === gameState.character.id);
  const totalWealth = gameState.character.money + ownedProperties.reduce((sum, p) => sum + p.price, 0);
  ...
  const newEvent: GameEvent = {
    ...
    description: `${gameState.character.name} has passed away. Their heir takes over with a net worth of $${totalWealth.toLocaleString()}.`,
